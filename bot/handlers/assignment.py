...
async def process_assignment_topic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await status_msg.delete()
        keyboard = [[InlineKeyboardButton("🔙 Back to Menu", callback_data="BACK_TO_MENU")]]
        await update.message.reply_text("⚠️ Failed to process your assignment.", reply_markup=InlineKeyboardMarkup(keyboard))
        return ConversationHandler.END
...
async def process_follow_up(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt = f"Based on the previous assignment analysis, answer this new question:\n\nCONTEXT:\n{history}\n\nNEW QUESTION:\n{follow_up_question}"
    try:
...
assignment_conversation_handler = ConversationHandler(
    ...
    fallbacks=[CommandHandler("cancel", cancel_assignment), CallbackQueryHandler(cancel_assignment, pattern="^BACK_TO_MENU$")]
)
