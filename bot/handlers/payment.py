...
async def show_subscription_plans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("💎 **Choose Your Plan**\n\nSelect a subscription plan to unlock all premium features.", reply_markup=reply_markup)
    return
...
async def start_plan_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"📧 **Email Required for {plan.name}**\n\nTo generate the payment link, please provide your email address. This is where your payment receipt will be sent.")
...
async def capture_email_and_generate_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if payment_url:
        keyboard = [[InlineKeyboardButton("💳 Pay Now (Secure Link)", url=payment_url)]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await update.message.reply_text("✅ **Link Ready!**\n\nClick the button below to complete your payment on Paystack's secure page. Your access will be upgraded automatically once the payment is confirmed.", reply_markup=reply_markup)
    else:
        await update.message.reply_text("⚠️ An error occurred while creating the payment link. Please try again later.")
